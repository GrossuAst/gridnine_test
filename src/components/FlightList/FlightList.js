...
export default function FlightList({ 
    ...
    // проверяет что в билете есть пересадка
    const ticketHasOneStop = (ticket) => {
        return ticket.flight.legs.every((leg) => leg.segments.length === 2)
    };

    // проверяет что рейс прямой
    const ticketHasNoStop = (ticket) => {
    ...
    return(
        <div className="container">
            <ul className="flight-list">
                {
                    // проверка выбраны ли чекбоксы пересадок
                    ticketsToShow.filter((ticket) => {
                        if(oneStopChecked && noStopsChecked) {
                            return true;
                        } else if(oneStopChecked) {
                            return ticketHasOneStop(ticket);
                        } else if(noStopsChecked) {
                            return ticketHasNoStop(ticket);
                        } else {
                            return true;
                        }
                    })
                    .slice(0, visibleTickets).map((f) => 
